...
export default function Certificate() {
  ...
  const name = profile?.display_name || user?.user_metadata?.display_name || 'Apprenant'
  ...
  return (<>
    <Helmet>
      <title>Mon Attestation | Formation Fortran 95</title>
      <meta name="description" content="Attestation de réussite de la formation Fortran 95 complète — 27 modules validés." />
    </Helmet>
    <div className="max-w-4xl mx-auto py-32">

      {!allDone && (
        <div className="alert alert-warning mb-6 no-print shadow-lg max-w-lg mx-auto">
          <span>🔒 Complétez les <strong>{stats.total}</strong> modules pour débloquer l'impression.</span>
      ...
      <div className="text-center mb-6 no-print">
        ...
        <Link to="/profile" className="btn btn-ghost btn-lg ml-3">← Retour</Link>
      </div>
      ...
      <div className="relative">
        ...
        <div id="certificate" className="relative bg-white overflow-hidden">
          {/* Bordures violet (#5C2D91) + or secondaire */}
          <div className="border-[12px] border-[#5C2D91] m-3">
            <div className="border-[3px] border-[#9B5DE5] m-1">
              <div className="border border-gray-200 m-1 p-8 md:p-12 text-center relative">
                ...
                <div className="absolute opacity-[0.04] pointer-events-none"
                     style={{fontSize: '500px', top: '50%', left: '50%', transform: 'translate(-50%,-50%)'}}>
                  F95
                </div>
                ...
                <div className="flex justify-between items-start mb-6">
                  ...
                  <div className="w-12 h-12 rounded-lg bg-[#5C2D91] flex items-center justify-center text-white font-bold text-sm">F95</div>
                </div>
                ...
                {/* Nom */}
                <p className="text-2xl md:text-4xl font-bold text-[#5C2D91] my-4 uppercase">
                  {name}
                </p>
                <div className="w-48 h-[3px] bg-[#9B5DE5] mx-auto mb-4" />
                ...
                <p className="text-sm md:text-xl font-semibold text-gray-800 italic mb-3">
                  « Formation Complète Fortran 95 — Du débutant à l'expert en calcul scientifique »
                </p>
                <p className="text-xs md:text-sm text-gray-500 max-w-2xl mx-auto leading-relaxed">
                  Variables, types, opérateurs, contrôle de flux, boucles, tableaux, fonctions, subroutines,
                  entrées/sorties, fichiers, modules, interfaces, allocation dynamique, types dérivés,
                  pointeurs, calcul numérique, OpenMP, BLAS/LAPACK et projet final.
                </p>

                {/* Statistiques */}
                <div className="flex justify-center gap-4 md:gap-12 my-6 md:my-8">
                  <div className="text-center">
                    <p className="text-xl md:text-3xl font-bold text-[#5C2D91]">{stats.total}</p>
                    <p className="text-[10px] md:text-xs text-gray-500">Modules complétés</p>
                  </div>
                  <div className="w-px bg-gray-200" />
                  <div className="text-center">
                    <p className="text-xl md:text-3xl font-bold text-[#5C2D91]">70+</p>
                    <p className="text-[10px] md:text-xs text-gray-500">Heures de formation</p>
                  </div>
                  <div className="w-px bg-gray-200" />
                  <div className="text-center">
                    <p className="text-xl md:text-3xl font-bold text-[#5C2D91]">4</p>
                ...
                <div className="flex justify-between items-end mt-8 md:mt-10">
                  ...
                  <div className="text-right flex flex-col items-center gap-1">
                    <img src="/images/210192671.png" alt="Formateur"
                         className="w-12 h-12 md:w-16 md:h-16 rounded-full object-cover border-2 border-[#5C2D91]"
                         onError={(e) => { e.target.style.display = 'none' }} />
                    <p className="text-[10px] md:text-xs text-gray-400">Le Formateur</p>
                    <p className="text-[9px] md:text-[10px] text-gray-400">Expert Fortran — Formation Complète</p>
                  </div>
                </div>
        ...
        <div className="text-center mt-4 no-print">
          <p className="text-xs text-base-content/40">
            Ce certificat est généré automatiquement et atteste de la complétion des 27 modules de la formation.
          </p>
        </div>
      </div>
    </div>
